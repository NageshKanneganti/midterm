...
class MultiplyCommand(Command):
    def __init__(self):
        super().__init__()
        self.name = "multiply"
        self.description = "Continuously multiply two numbers. Type 'exit' to return to the main menu."

    def execute(self, *args):
        print("Operation: Multiplication")
        print("\tContinuously enter two numbers separated by space to perform multiplication.")
        print("\tType 'exit' at any time to return to the main menu.")
        print("\t\t[Example]: 3 3")

        while True:
            user_input = input("[Multiply]:  ")

            if user_input.lower() == 'exit':
                print("Exiting multiplication operation.\n")
                break

            try:
                num1_str, num2_str = self.parse_input(user_input)
                calculate_and_print(num1_str, num2_str, "multiply")
                print("You can continue multiplying or type 'exit' to return to the main menu.\n")
